...
import { getQuizzes, getDashboard, startQuiz } from "@/lib/api";
...
function ScoreBar({ percentage }: { percentage: number }) {
  ...
  return (
    <div className="flex items-center gap-3">
      ...
      <span className={`text-xs font-bold tabular-nums ${isPassed ? "text-emerald-600" : "text-amber-600"}`}>
      ...
      <header className="sticky top-0 z-40 glass border-b border-white/30">
        <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between">
          <Link href="/" className="flex items-center gap-2.5 group">
            <div className="w-9 h-9 rounded-xl bg-gradient-to-br from-emerald-500 to-teal-500 flex items-center justify-center shadow-md shadow-emerald-500/20 group-hover:shadow-lg group-hover:shadow-emerald-500/30 transition-all duration-200">
          ...
          <div className="relative flex flex-col sm:flex-row items-start sm:items-center justify-between gap-6">
            <div>
              ...
              <p className="text-emerald-100/80 text-sm sm:text-base max-w-md">
                {availableQuizzes > 0
                  ? `You have ${availableQuizzes} assessment${availableQuizzes > 1 ? "s" : ""} waiting for you. Stay focused and give it your best!`
                  : "You've completed all available assessments. Great work! 🎉"}
              </p>
            </div>
            {completedCount > 0 && (
              <div className="flex-shrink-0">
                <ProgressRing value={Math.round(avgScore)} size={80} stroke={6} />
              </div>
            )}
          </div>
        </div>
                        ...
                        <div className="flex items-center gap-4 flex-1 min-w-0">
                          ...
                          <div className="min-w-0">
                            ...
                            <div className="flex items-center gap-2.5 mt-1.5 flex-wrap">
                              ...
                              {!quiz.canStart && hasScore && (
                                <span className={`inline-flex items-center gap-1 px-2 py-0.5 rounded-md text-xs font-bold ${
                                  quizHistory!.percentage >= 75
                                    ? "bg-emerald-100 text-emerald-700"
                                    : "bg-amber-100 text-amber-700"
                                }`}>
                                  Score: {quizHistory!.percentage.toFixed(0)}%
                                </span>
                              )}
                            </div>
                          </div>
                        ...
                        <div className="flex-shrink-0 sm:ml-4">
                          {quiz.canStart ? (
                            ...
                            <div className="flex items-center gap-2">
                              <ScoreBar percentage={quizHistory?.percentage ?? 0} />
                              <span className="inline-flex items-center gap-1 px-3 py-2 rounded-xl text-xs font-bold bg-blue-50 text-blue-600 border border-blue-100">
          ...
          <section className="animate-fade-in">
            {history.length === 0 ? (
              ...
              <div className="space-y-3">
                {history.map((entry, i) => {
                  const isPassed = entry.status === "Passed" || entry.percentage >= 75;
                      ...
                      >
                        ...
                        <div className="flex items-center gap-4">
                          <div className="text-right">
                            <ProgressRing value={entry.percentage} size={48} stroke={3} color={isPassed ? "emerald" : "amber"} />
                          </div>
                          <svg
                            ...
                            {entry.answers.map((ans, idx) => (
                              <div
                                ...
                                className={`rounded-xl p-4 border transition-colors ${
                                  ans.isCorrect
                                    ? "bg-emerald-50/50 border-emerald-200/50 hover:border-emerald-300/50"
                                    : "bg-rose-50/30 border-rose-200/50 hover:border-rose-300/50"
                                }`}
                              >
